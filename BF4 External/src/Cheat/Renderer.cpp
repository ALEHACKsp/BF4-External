...
void Renderer::RenderLoop(Direct2DOverlay* overlay)
{
	...
	for (unsigned int idx = 0; idx < 70; idx++)
	{
		...
		uintptr_t occluded = M.Read<uintptr_t>(soldier + Offsets::OCCLUDED);
		...
		{
			float bh = head_screen.y - origin_screen.y;
			float bw = bh / 2.f;

			float green = HP * 2.55;
			float red = 255 - green;
			float health_bar = (bw * HP) / 100.0f * 2;
			

			if (!is_team && soldier && HP > 0)
			{
				if (bw >= 80.0f)
					continue;
				...
				overlay->DrawCircle({ head_screen.x, head_screen.y }, 1, 1, render_colour, false);
				...
				//overlay->DrawBox({ origin_screen.x - 10 , origin_screen.y - 1 }, { origin_screen.x + 10, origin_screen.y + 3 }, 1, { 255,255,255,255 }, false);
				//overlay->DrawBox({ origin_screen.x - 9 , origin_screen.y }, { origin_screen.x + 9, origin_screen.y + 2 }, 1, { 0,255,0,255 }, true);
				overlay->DrawString(std::to_wstring(static_cast<int>(HP)), 9, { origin_screen.x - bw, origin_screen.y + 5 }, ESP_FONT, FONT_CENTER, { 255, 255, 255, 255 });
			}
